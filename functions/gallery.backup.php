...
function corner_gallery_shortcode($attr) {
	...
	foreach ( $ids as $id ) {
		...
		if ( slt_cf_field_value('disable-zoom') ) {
			...
			$output .= '
			<div class="rsContent">
				<a class="zoom-img" href="'.$image_large[0].'" rel="shadowbox['.$post->post_name.'-alt]" title="'.get_the_title($post->ID).'"><img src="'.$image[0].'" alt="" /></a>
			</div>';
		}		
	...
	if ( !slt_cf_field_value('disable-zoom') ) {
		...
		foreach ( $ids as $id ) {
			...
			if ($i == 0) {
				...
				$output .= '<a class="zoom '.$additional_classes.'" href="'.$href.'" '.$rel.' title="'.get_the_title($post->ID).'"><img src="'.$zoom_button_url.'" alt="Zoom" height="25" width="25"></a>';			
	
			} else {
				$output .= '<a style="display:none" class="zoom" href="'.$image_large[0].'" rel="shadowbox['.$post->post_name.']" title="'.get_the_title($post->ID).'"><img src="http://thecornerlondon.com/site2.0/images/zoom.png" alt="Zoom" height="25" width="25"></a>';
